    ...
    public function renderWidget(AbstractWidget $widget, RenderContext $context): array
    {
        ...
        foreach ($lines as $i => $line) {
            ...
            if ($lineWidth > $availableColumns) {
                throw new RenderException(\sprintf('Widget "%s" rendered line %d with width %d, exceeding the available %d columns. Line preview: %s.', $widget::class, $i, $lineWidth, $availableColumns, mb_substr(AnsiUtils::stripAnsiCodes($line), 0, 100)), $i, $lineWidth, $availableColumns);
            }
        }
